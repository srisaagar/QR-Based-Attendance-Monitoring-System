...
const TeacherDashboard = () => {
  ...
  useEffect(() => {
  const fetchTokenForExistingSession = async () => {
    if (!sessionId || !localIP) return;

    try {
      const res = await axios.get(`http://localhost:5000/api/attendance/${sessionId}`);
      if (res.data && res.data.qrCodeToken) {
        setQrValue(`http://${localIP}:3000/attendance/${sessionId}?token=${res.data.qrCodeToken}`);
      } else {
        console.warn('No QR token found in session data');
      }
    } catch (err) {
      console.error('Failed to fetch session data for QR:', err);
    }
  };

  fetchTokenForExistingSession();
}, [sessionId, localIP]);
  ...
  useEffect(() => {
    ...
    const refreshQR = async () => {
      try {
        const response = await axios.put(`http://localhost:5000/api/attendance/refresh-qr/${sessionId}`);
        setQrValue(`http://${localIP}:3000/attendance/${sessionId}?token=${response.data.qrCodeToken}`);
      } catch (error) {
      ...
      {showQrModal && (
        <div className="qr-modal-overlay" onClick={() => setShowQrModal(false)}>
          <div className="qr-modal" onClick={(e) => e.stopPropagation()}>
            ...
            <p className="text-muted"><b>QR auto-refreshes every 15 seconds</b></p>
          </div>
        </div>
      )}
    </div>
